...
void Parser::preprocessProgram(string program)
{
	...
	do {
		...
		thisStmt.erase(remove(thisStmt.begin(), thisStmt.end(), '\n'), thisStmt.end());

		//Parse thisStmt
		if (thisStmt.size() > 0) {
			regex procRegex("\\s*procedure\\s*([a-zA-Z0-9]*)\\s*");
			regex whileRegex("\\s*while\\s*([a-zA-Z0-9]*)\\s*");
			regex assignRegex("\\s*([a-zA-Z0-9]+)\\s*=\\s*((\\(|[a-zA-Z0-9]+)+(\\s*[\\+|\\-|\\*|\\(]\\s*\\(*\\s*[a-zA-Z0-9]+\\s*\\)*)*)");
			smatch sm;
			...
			if (regex_match(thisStmt, sm, procRegex)) {
				...
				cout << "Parser cannot parse this program." << endl;
			}
